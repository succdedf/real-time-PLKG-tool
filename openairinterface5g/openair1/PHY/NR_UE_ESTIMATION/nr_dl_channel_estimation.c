...
#ifdef DEBUG_PDSCH
    int k0 = bwp_start_subcarrier % ue->frame_parms.ofdm_symbol_size;
    printf("\n==========================\n");
    printf("Frame %d, Slot %d, Symbol %i\n", proc->frame_rx, Ns, symbol);
    printf("============================\n");
    printf("UE Subcarrier = %d\n", k0);
...
//#ifdef DEBUG_PDSCH 
    ...
    if(k0 == 900 && symbol == 2){
          ...
          dl_ch = (c16_t *)&dl_ch_estimates[p * ue->frame_parms.nb_antennas_rx + aarx][ch_offset];
          for (int i = 0; i < 60; i++) {
              dmrs_magnitude[i] = sqrt(dl_ch[i].r*dl_ch[i].r + dl_ch[i].i*dl_ch[i].i);
              printf("%d ", dmrs_magnitude[i]);
              //printf("%d+%dj ", dl_ch[i].r, dl_ch[i].i);
              //printf("%d ",dl_ch[i].r);
          }
          printf("\n");
    }
